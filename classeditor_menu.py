...
from classeditor_dialogs import *

# ---------------------------------------------

class classeditor_menu:
	def __init__(self, iface):
		self.iface = iface

	def initGui(self):
		icon = QIcon(os.path.dirname(__file__) + "/icons/pmn_logo.png")
		...
		self.iface.registerMainWindowAction(self.update_selected_action, "F11")  # self.update_selected_action is triggered by the F12
	       	self.iface.addToolBarIcon(self.update_selected_action)
        	self.iface.addPluginToMenu("&classeditor", self.update_selected_action)
	...
	def unload(self):
		self.iface.unregisterMainWindowAction(self.update_selected_action)
    		self.iface.removePluginMenu("&classeditor", self.update_selected_action)

	def update_selected(self):
		dialog = classeditor_update_selected_dialog(self.iface)
		dialog.exec_()
